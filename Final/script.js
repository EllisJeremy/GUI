...
function displayItems(items) {
    ...
    itemsToDisplay.forEach(item => {
        ...
        col.innerHTML = `
            ...
            <img src="inventory/${item.image}" class="card-img-top" alt="${item.name}" style="height: 120px; object-fit: cover;">
                <div class="card-body">
...
function handleSearch() {
    ...
    if (searchTerm === '') {
        ...
        filteredItems = allItems.map(item => item.name.toUpperCase().includes(searchTerm) ? item : null).filter(item => item !== null);
    }
    displayItems(filteredItems);
}
...
function updateCartModal() {
    ...
    cart.forEach(item => {
        ...
        itemDiv.innerHTML = `
            <img src="inventory/${item.image}" alt="${item.name}" style="width: 60px; height: 60px; object-fit: cover;" class="me-3">
...
document.getElementById('closeModalBtn').addEventListener('click', hideCartModal);
